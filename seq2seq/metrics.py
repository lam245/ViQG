...
class BleuScorer(object):
    ...
    def data_score(self, data, predictor):
        ...
        results = [max((v for v in results_prelim if v['question'] == x), key=lambda y: y['bleu_1']) for x in
                   set(v['question'] for v in results_prelim)]
    ...
    def data_meteor_score(self, data, predictor):
        ...
        results_meteor = [max((v for v in results_prelim if v['question'] == x), key=lambda y: y['meteor_score']) for x
                          in set(v['question'] for v in results_prelim)]

        with open('result_meteor_output.txt', 'w',  encoding = 'utf-8') as f:
